...
Before re-adding the framework, make sure the "Framework Search Paths" property of your target does not point to the older version of Zooz SDK

Also need to add AdSupport.framework to the project linked libraries
********************************************************************************
...
1) Removed dependency in AdSupport.framework due Apple restrictions.


SDK version 1.5
----------------
1) Full support for iOS7

2) Updated PayPal package with support for iOS7

3) Updated Netswipe package (For camera scan) with support for iOS7

4) Removed deprecated API in Zooz class - -(BOOL)preInitialize:(NSString *)appKey isSandboxEnv:(BOOL)isSandbox
...
SDK version 1.4.6
------------------
1) Added subscription option.

2) Added Chinese (Traditional and simplified) and Albanian localization.

3) preInitialize:isSandboxEnv: is deprecated. Please use preInitialize:isSandboxEnv:withCurrencyCode: instead.
...
SDK version 1.4.5
------------------
1) PayPal is back with support to new Apple UDID limitation.

2) Small French translation fixes.


SDK version 1.4.4
------------------
1) PayPal is disabled, because of Apple UDID limitation.

2) Removed references to Apple unique identifier.

3) New UI, fonts and look and feel

4) Now users can override font family using zooz property.
